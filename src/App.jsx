...
import {
  CAT_DEFS, SWATCHES, SEED, MONTHS, MONTHS_BY_LANG, CAT_NAMES_BY_LANG, I18N,
} from './data.js';
import {
  pad, isoOf, defaultCatColor, coinFace, formatNum, fmtMoney, fmtShort,
  downloadFile, csvEscape, fuzzyMatch, parseCsv,
} from './utils.js';
...
export default class App extends React.Component {
  constructor(props) {
    ...
    } catch (err) { /* ignore corrupt/unavailable storage */ }
  }
  ...
  componentDidMount() {
    ...
    const s = this.state;
    this._persistSnapshot = { expenses: s.expenses, categories: s.categories, language: s.language, currency: s.currency, rates: s.rates, numberFormat: s.numberFormat, listGrouping: s.listGrouping, period: s.period };
  }
  ...
  componentDidUpdate() {
    ...
    if (!prev || prev.expenses !== s.expenses || prev.categories !== s.categories ||
        prev.language !== s.language || prev.currency !== s.currency ||
        prev.rates !== s.rates || prev.numberFormat !== s.numberFormat || prev.listGrouping !== s.listGrouping ||
    ...
    switch (this.state.period) {
      ...
      case 'week': {
        ...
        if (start.getFullYear() === end.getFullYear() && start.getMonth() === end.getMonth()) {
          return pad(start.getDate()) + ' \u2013 ' + pad(end.getDate()) + ' ' + months[end.getMonth()] + " '" + String(end.getFullYear()).slice(2);
        }
        return fmtShort(isoOf(start), lang, MONTHS_BY_LANG, MONTHS) + ' \u2013 ' + fmtShort(isoOf(end), lang, MONTHS_BY_LANG, MONTHS);
      }
      case 'year': return String(y + off);
      case 'month':
      default: {
        const dt = new Date(y, m + off, 1);
  ...
  getFilteredExpenses() {
    ...
    const catById = {};
    this.state.categories.forEach((c) => { catById[c.id] = c; });
    const query = (this.state.searchQuery || '').trim().toLowerCase();
    if (query.length > 0) {
      return this.state.expenses.filter((e) => {
        const cat = catById[e.categoryId] || { id: 'other', name: 'Other' };
        const hay = [e.note, this.catLabel(cat), fmtShort(e.date, lang, MONTHS_BY_LANG, MONTHS), e.date, String(e.amount)].join(' ').toLowerCase();
  ...
  playChaChing() {
    try {
      ...
      const notes = [[1046.5, 0, 0.09], [1568, 0.07, 0.14], [2093, 0.16, 0.22]];
  ...
  persist() {
    ...
    } catch (err) { /* storage unavailable (private mode, quota) */ }
  }
  ...
  selectPeriod = (p) => {
    ...
    this.setState((s) => ({ period: p, periodOffset: 0, swipeDir: dir === 0 ? 1 : (dir > 0 ? 1 : -1), lastAction: 'period', swipeTick: (s.swipeTick || 0) + 1 }));
  };
  shiftPeriod = (dir) => this.setState((s) => ({ periodOffset: s.periodOffset + dir, swipeDir: dir, lastAction: 'period', swipeTick: (s.swipeTick || 0) + 1 }));
  ...
  goHome = () => this.setState((s) => ({ screen: 'home', swipeDir: 0, lastAction: 'toHome', swipeTick: (s.swipeTick || 0) + 1 }));
  goGraph = () => this.setState((s) => ({ screen: 'graph', swipeDir: 0, lastAction: 'toGraph', swipeTick: (s.swipeTick || 0) + 1 }));
  goSettings = () => this.setState((s) => ({ screen: s.screen === 'settings' ? 'home' : 'settings' }));
  setListGrouping = (mode) => this.setState((s) => ({ listGrouping: mode, swipeDir: 0, lastAction: 'period', swipeTick: (s.swipeTick || 0) + 1 }));
  setHoverCat = (id) => this.setState({ hoverCatId: id });
  clearHoverCat = () => this.setState({ hoverCatId: null });
  toggleSearch = () => this.setState((s) => ({ searchOpen: !s.searchOpen, searchQuery: s.searchOpen ? '' : s.searchQuery }));
  ...
  exportCsv = () => {
    const catById = {}; this.state.categories.forEach((c) => { catById[c.id] = c; });
  ...
  onImportJsonFile = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        const catByName = {};
        this.state.categories.forEach((c) => { catByName[c.name.toLowerCase()] = c.id; });
  ...
  onImportCsvFile = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        const dIdx = header.indexOf('date'), cIdx = header.indexOf('category'), aIdx = header.indexOf('amount'), nIdx = header.indexOf('note');
        const catNames = new Set(rows.slice(1).map((r) => (r[cIdx] || '').trim()).filter(Boolean));
  ...
  mergeImportedCategories(list) {
    ...
    list.forEach((c, i) => {
      ...
      if (name && !existingNames.has(name.toLowerCase())) {
        ...
        toAdd.push({ id: 'custom_' + Date.now() + '_' + i, name, color: SWATCHES[(this.state.categories.length + toAdd.length) % SWATCHES.length] });
      }
    });
  ...
  openAdd = () => this.setState({ showAdd: true, sheetClosing: false, editingId: null, addAmount: '', addDate: isoOf(TODAY), addCategoryId: 'food', addNote: '', addingCategory: false, categoryPickerOpen: false });
  requestCloseAdd = () => {
    if (this.state.sheetClosing) return;
    this.setState({ sheetClosing: true });
    if (this._sheetCloseTimer) clearTimeout(this._sheetCloseTimer);
    this._sheetCloseTimer = setTimeout(() => {
      this.setState({ showAdd: false, sheetClosing: false, addingCategory: false, editingId: null, categoryPickerOpen: false });
    }, 220);
  };
  requestEditEntry = (entry) => this.setState({
    showAdd: true, editingId: entry.id, addingCategory: false, categoryPickerOpen: false,
    addAmount: String(entry.amount), addDate: entry.date, addCategoryId: entry.categoryId, addNote: entry.note || '',
  });
  ...
  submitAdd = () => {
    ...
    if (this.state.editingId) {
      ...
      this.setState((s) => ({
        ...
        showAdd: false, addAmount: '', addNote: '', addingCategory: false, editingId: null,
      }));
      this.playChaChing();
      this.showToast(I18N[this.state.language]?.savedToast || 'Saved');
    } else {
      const id = 'e' + Date.now();
      const entry = { id, amount: amt, date: this.state.addDate || isoOf(TODAY), categoryId: this.state.addCategoryId, note: (this.state.addNote || '').trim() };
      this.setState((s) => ({ expenses: [entry, ...s.expenses], showAdd: false, addAmount: '', addNote: '', addingCategory: false, lastAddedId: id }));
      this.playChaChing();
      this.showToast(this.convertAndFormat(amt, this.state.currency) + ' ' + (I18N[this.state.language]?.addedToast || 'added'));
    }
  };

  requestDelete = (id) => this.setState({ deleteModalId: id });
  cancelDeleteModal = () => this.setState({ deleteModalId: null });
  confirmDeleteModal = () => this.setState((s) => ({ expenses: s.expenses.filter((e) => e.id !== s.deleteModalId), deleteModalId: null }));
  ...
  rowSwipeEnd = (id) => {
    ...
    this._rowSwipeX = null; this._rowSwipeId = null;
    if (offset < -60) {
  ...
  confirmDeleteAll = () => {
    ...
    this.setState({
      expenses: [],
      categories: CAT_DEFS.map((c, i) => ({ id: c.id, name: c.name, color: defaultCatColor(i) })),
      deleteAllOpen: false, deleteAllText: '', screen: 'settings',
    });
  };
  ...
  getViewData() {
    ...
    const mkRow = (r) => {
      ...
      return {
        ...
        requestDelete: (e) => { if (e && e.stopPropagation) e.stopPropagation(); this.requestDelete(r.id); },
    ...
    if (grouping === 'category') {
      ...
      groupedList = Array.from(byCat.entries())
        .map(([catId, rows]) => {
          ...
          return { key: catId, dateLabel: this.catLabel(cat), showHeader: true, sortKey: sum, rows: rows.map((r) => ({ ...mkRow(r), showDateInline: true })) };
        })
        .sort((a, b) => b.sortKey - a.sortKey);
    } else if (grouping === 'none') {
      groupedList = filtered.length ? [{ key: 'all', dateLabel: '', showHeader: false, rows: filtered.map((r) => ({ ...mkRow(r), showDateInline: true })) }] : [];
    } else {
    ...
    const catArr = Array.from(catTotals.entries()).map(([id, amt]) => ({ id, amt, cat: catById[id] || { id: 'other', name: 'Other', color: '#888' } }))
    ...
    const periodDefs = [
      ['day', t.periodDay], ['week', t.periodWeek], ['month', t.periodMonth], ['year', t.periodYear],
    ];
    const ink = '#2c2416', accent = '#b5432e', paperFg = '#f4ecd8';
    ...
    const categoriesForPicker = this.state.categories.map((c) => {
      ...
      return {
        id: c.id, name: label, initial: label.charAt(0).toUpperCase(), face: coinFace(c.color),
    ...
    const swatches = SWATCHES.map((hex, i) => ({
      hex, select: () => this.setState({ newCatColor: i }),
    ...
    const nfActive = ink, nfInactive = 'transparent';
    const thousandsToggle = [
      { id: true, label: t.on }, { id: false, label: t.off },
    ].map((o) => ({ ...o, select: () => this.setNumberFormat({ thousands: o.id }), bg: numberFormat.thousands === o.id ? nfActive : nfInactive, fg: numberFormat.thousands === o.id ? paperFg : ink }));
    const thousandsCharOptions = [
      { id: ',', label: ',' }, { id: '.', label: '.' },
    ].map((o) => ({ ...o, select: () => this.setNumberFormat({ thousandsChar: o.id }), bg: numberFormat.thousandsChar === o.id ? nfActive : nfInactive, fg: numberFormat.thousandsChar === o.id ? paperFg : ink }));
    const decimalsOptions = [
      { id: 0, label: '0' }, { id: 2, label: '2' },
    ].map((o) => ({ ...o, select: () => this.setNumberFormat({ decimals: o.id }), bg: numberFormat.decimals === o.id ? nfActive : nfInactive, fg: numberFormat.decimals === o.id ? paperFg : ink }));
    const decimalCharOptions = [
      { id: '.', label: '.' }, { id: ',', label: ',' },
    ].map((o) => ({ ...o, select: () => this.setNumberFormat({ decimalChar: o.id }), bg: numberFormat.decimalChar === o.id ? nfActive : nfInactive, fg: numberFormat.decimalChar === o.id ? paperFg : ink }));
    const numberFormatPreview = formatNum(1234.5, numberFormat);

    return {
      lang, cur, t, catById, isSearching, filtered, hasEntries,
      groupedList, pieSegments, periods, groupingOptions, donutCenterLabel, donutCenterValue,
      categoriesForPicker, selectedCatObj, selectedCatLabel, swatches,
      languageOptions, currencyOptions,
      deleteEntry, deleteCat,
      numberFormat, thousandsToggle, thousandsCharOptions, decimalsOptions, decimalCharOptions, numberFormatPreview,
      contentAnim: this.state.lastAction === 'toHome' ? 'cbLedgerIn 0.55s cubic-bezier(.22,1,.36,1)'
        : this.state.lastAction === 'toGraph' ? 'cbChartIn 0.5s cubic-bezier(.22,1,.36,1)'
        : this.state.swipeDir > 0 ? 'cbSlideInLeft 0.42s cubic-bezier(.2,.8,.3,1)'
        : this.state.swipeDir < 0 ? 'cbSlideInRight 0.42s cubic-bezier(.2,.8,.3,1)'
        ...
        <div className="cb-phone">
          ...
          <div className="cb-app">
            ...
            <div className="cb-header">
              <div className="cb-gear hover-gear" onClick={this.goSettings}>⚙</div>
              {notSettings && (
                <div className="cb-search-btn hover-chevron" onClick={this.toggleSearch}>
                  <svg width="15" height="15" viewBox="0 0 15 15"><circle cx="6.5" cy="6.5" r="5" fill="none" stroke="#2c2416" strokeWidth="1.6" /><line x1="10.3" y1="10.3" x2="14" y2="14" stroke="#2c2416" strokeWidth="1.8" strokeLinecap="round" /></svg>
                </div>
              )}
                ...
                <>
                  {v.isSearching && (
                    <div style={{ textAlign: 'center', padding: '10px 0 4px' }}>
                      <div style={{ fontSize: 11, letterSpacing: 1, color: '#8a7355' }}>{v.filtered.length} {v.filtered.length === 1 ? 'result' : 'results'} for "{s.searchQuery}"</div>
                    </div>
                  )}
                  ...
                  {notSearching && (
                    <>
                      <div className="cb-period-row">
                        {v.periods.map((p) => (
                          <div key={p.id} className="cb-period-tab hover-lift" onClick={p.select}
                            style={{ border: p.border, background: p.bg, color: p.fg, boxShadow: p.shadow, transform: p.scale }}>
                            <PeriodIcon id={p.id} />
                            {p.label}
                          </div>
                        ))}
                      ...
                      <div className="cb-total-wrap" onTouchStart={this.handleSwipeStart} onTouchEnd={this.handleSwipeEnd} onMouseDown={this.handleSwipeStart} onMouseUp={this.handleSwipeEnd}>
                        <div className="cb-ticket">
                          ...
                          <div className="cb-period-nav">
                            ...
                            <div className="cb-chevron hover-chevron" onClick={() => this.shiftPeriod(-1)}>‹</div>
            ...
            <div className="cb-footer">
              <div className="cb-nav-btn hover-nav press-96" onClick={this.goHome} style={{ color: isHome ? '#b5432e' : '#8a7355' }}>
                <div className="cb-nav-icon-wrap" style={{ background: isHome ? 'rgba(181,67,46,0.14)' : 'transparent', transform: isHome ? 'scale(1.08)' : 'scale(1)' }}>
                  <svg width="16" height="16" viewBox="0 0 16 16"><rect x="1" y="2" width="14" height="2" rx="1" fill="currentColor" /><rect x="1" y="7" width="14" height="2" rx="1" fill="currentColor" /><rect x="1" y="12" width="9" height="2" rx="1" fill="currentColor" /></svg>
                </div>
                <div className="cb-nav-label">{t.navLedger}</div>
              </div>
              <div className="cb-fab hover-fab" onClick={this.openAdd}>+</div>
              <div className="cb-nav-btn hover-nav press-96" onClick={this.goGraph} style={{ color: isGraph ? '#b5432e' : '#8a7355' }}>
                <div className="cb-nav-icon-wrap" style={{ background: isGraph ? 'rgba(181,67,46,0.14)' : 'transparent', transform: isGraph ? 'scale(1.08)' : 'scale(1)' }}>
                  <svg width="16" height="16" viewBox="0 0 16 16"><circle cx="8" cy="8" r="6.6" fill="none" stroke="currentColor" strokeWidth="1.4" /><path d="M8 8 L8 1.4 A6.6 6.6 0 0 1 13.7 11 Z" fill="currentColor" /></svg>
                </div>
            ...
            <>
              ...
              <div className="cb-search-popup">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input type="text" placeholder="Search notes, categories, dates…" value={s.searchQuery} onChange={this.onSearchInput} autoFocus className="cb-input cb-input-pill" />
                  <div className="cb-round-btn hover-delete" onClick={this.closeSearch}>✕</div>
                </div>
                {v.isSearching && <div style={{ fontSize: 10.5, letterSpacing: 1, color: '#8a7355', marginTop: 8, paddingLeft: 4 }}>{v.filtered.length} {v.filtered.length === 1 ? 'result' : 'results'}</div>}
              </div>
            </>
          )}
          ...
          {s.deleteModalId && (
            <>
              ...
              <div className="cb-modal">
                ...
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 2 }}>{v.deleteCat ? this.catLabel(v.deleteCat) : ''} — {v.deleteEntry ? this.convertAndFormat(v.deleteEntry.amount, s.currency) : ''}</div>
                <div style={{ fontSize: 12, color: '#8a7355', marginBottom: 18 }}>{v.deleteEntry ? fmtShort(v.deleteEntry.date, s.language, MONTHS_BY_LANG, MONTHS) : ''}</div>
                <div style={{ display: 'flex', gap: 10 }}>
                  <div className="cb-modal-btn cb-modal-btn-danger hover-lift" onClick={this.confirmDeleteModal}>{t.yes}</div>
                  <div className="cb-modal-btn cb-modal-btn-outline hover-lift" onClick={this.cancelDeleteModal}>{t.no}</div>
                </div>
              </div>
            </>
          )}
          ...
          {s.deleteAllOpen && (
            <>
              ...
              <div className="cb-modal cb-modal-danger">
                ...
                <input type="text" placeholder={t.typeYesPlaceholder} value={s.deleteAllText} onChange={this.onDeleteAllTextInput} className="cb-input cb-input-danger" />
                <div style={{ display: 'flex', gap: 10 }}>
                  <div className="cb-modal-btn cb-modal-btn-danger" style={{ opacity: s.deleteAllText === 'Yes' ? 1 : 0.4, cursor: s.deleteAllText === 'Yes' ? 'pointer' : 'default' }} onClick={this.confirmDeleteAll}>{t.eraseEverything}</div>
                  <div className="cb-modal-btn cb-modal-btn-outline hover-lift" onClick={this.cancelDeleteAll}>{t.cancel}</div>
                </div>
              </div>
            </>
          )}
...
function PeriodIcon({ id }) {
  if (id === 'day') return <svg width="14" height="14" viewBox="0 0 14 14"><circle cx="7" cy="7" r="3" fill="currentColor" /><line x1="7" y1="0.5" x2="7" y2="2.2" stroke="currentColor" strokeWidth="1.3" /><line x1="7" y1="11.8" x2="7" y2="13.5" stroke="currentColor" strokeWidth="1.3" /><line x1="0.5" y1="7" x2="2.2" y2="7" stroke="currentColor" strokeWidth="1.3" /><line x1="11.8" y1="7" x2="13.5" y2="7" stroke="currentColor" strokeWidth="1.3" /></svg>;
  if (id === 'week') return <svg width="16" height="14" viewBox="0 0 16 14"><rect x="0" y="3" width="1.6" height="8" fill="currentColor" /><rect x="2.4" y="1" width="1.6" height="12" fill="currentColor" /><rect x="4.8" y="4" width="1.6" height="6" fill="currentColor" /><rect x="7.2" y="0" width="1.6" height="14" fill="currentColor" /><rect x="9.6" y="3" width="1.6" height="8" fill="currentColor" /><rect x="12" y="1" width="1.6" height="12" fill="currentColor" /><rect x="14.4" y="4" width="1.6" height="6" fill="currentColor" /></svg>;
  if (id === 'month') return <svg width="14" height="14" viewBox="0 0 14 14"><rect x="0.5" y="1.5" width="13" height="12" rx="1.4" fill="none" stroke="currentColor" strokeWidth="1.2" /><line x1="0.5" y1="5" x2="13.5" y2="5" stroke="currentColor" strokeWidth="1.2" /><line x1="4" y1="0" x2="4" y2="2.5" stroke="currentColor" strokeWidth="1.2" /><line x1="10" y1="0" x2="10" y2="2.5" stroke="currentColor" strokeWidth="1.2" /></svg>;
  return <svg width="14" height="14" viewBox="0 0 14 14"><circle cx="7" cy="7" r="6.3" fill="none" stroke="currentColor" strokeWidth="1.2" /><line x1="7" y1="0.7" x2="7" y2="13.3" stroke="currentColor" strokeWidth="1.2" /><line x1="0.7" y1="7" x2="13.3" y2="7" stroke="currentColor" strokeWidth="1.2" /></svg>;
}
...
function CoinScatter({ tick }) {
  ...
  return (
    <>
      <div key={'cluster' + tick} className="cb-coin-cluster" style={{ animation: 'cbCoinBurst 0.6s cubic-bezier(.3,.6,.3,1)' }}>
      ...
      {coins.slice(1).map((c) => (
        <div key={c.key + tick} className={'cb-coin-scatter ' + c.cls} style={{ animation: `cbCoinBurst 0.6s cubic-bezier(.3,.6,.3,1) ${c.delay}s both` }}>
    ...
    <div style={{ animation: anim }}>
      <div className="cb-group-row">
        {v.groupingOptions.map((g) => (
          <div key={g.id} className="cb-group-tab hover-lift" onClick={g.select} style={{ border: g.border, background: g.bg, color: g.fg, boxShadow: g.shadow }}>
            <GroupIcon id={g.id} />
            {g.label}
          </div>
        ))}
          ...
          <React.Fragment key={grp.key}>
            ...
            {grp.rows.map((row) => (
              <div key={row.id} className="cb-row-outer">
                <div className="cb-row-delete-bg"><span>✕</span></div>
                <div
                  className="cb-row hover-lift press-96"
                  onClick={row.requestEdit}
                  onTouchStart={row.swipeStart} onTouchMove={row.swipeMove} onTouchEnd={row.swipeEnd}
                  style={{ background: row.rowBg, transform: row.rowTransform, animation: row.stampAnim, transition: row.rowTransition }}
                >
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#b5432e', whiteSpace: 'nowrap' }}>{row.amountStr}</div>
                  <div className="cb-round-btn cb-round-btn-sm hover-delete press-96" onClick={row.requestDelete}>✕</div>
                </div>
              </div>
            ))}
          </React.Fragment>
...
function GroupIcon({ id }) {
  if (id === 'date') return <svg width="12" height="12" viewBox="0 0 14 14"><rect x="0.5" y="1.5" width="13" height="12" rx="1.4" fill="none" stroke="currentColor" strokeWidth="1.2" /><line x1="0.5" y1="5" x2="13.5" y2="5" stroke="currentColor" strokeWidth="1.2" /><line x1="4" y1="0" x2="4" y2="2.5" stroke="currentColor" strokeWidth="1.2" /><line x1="10" y1="0" x2="10" y2="2.5" stroke="currentColor" strokeWidth="1.2" /></svg>;
  if (id === 'category') return <svg width="12" height="12" viewBox="0 0 14 14"><rect x="0.5" y="0.5" width="6" height="6" rx="1" fill="currentColor" /><rect x="7.5" y="0.5" width="6" height="6" rx="1" fill="currentColor" opacity="0.55" /><rect x="0.5" y="7.5" width="6" height="6" rx="1" fill="currentColor" opacity="0.55" /><rect x="7.5" y="7.5" width="6" height="6" rx="1" fill="currentColor" /></svg>;
  return <svg width="14" height="12" viewBox="0 0 14 12"><rect x="0" y="1" width="14" height="1.8" rx="0.9" fill="currentColor" /><rect x="0" y="5.1" width="14" height="1.8" rx="0.9" fill="currentColor" /><rect x="0" y="9.2" width="14" height="1.8" rx="0.9" fill="currentColor" /></svg>;
}
    ...
    <div style={{ animation: anim }}>
      <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', padding: '14px 0 6px' }}>
        <div style={{ position: 'relative', width: 188, height: 188 }}>
          <svg width="188" height="188" viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)', filter: 'drop-shadow(0 3px 5px rgba(0,0,0,0.2))', animation: 'cbPieIn 0.45s ease-out' }}>
            <circle cx="50" cy="50" r="38" fill="none" stroke="#dcd0ae" strokeWidth="19" />
            {v.pieSegments.map((seg) => (
              <circle key={seg.id} onMouseEnter={seg.onEnter} onMouseLeave={seg.onLeave} cx="50" cy="50" r="38" fill="none"
                stroke={seg.strokeColor} strokeWidth={seg.strokeWidth} strokeDasharray={seg.dasharray} strokeDashoffset={seg.dashoffset}
                strokeLinecap="butt" opacity={seg.opacity}
                style={{ cursor: 'pointer', transition: 'stroke-width 0.15s ease, opacity 0.15s ease, stroke 0.15s ease', animation: `cbSegFade 0.45s ease-out ${seg.delayMs}ms both` }} />
            ))}
          </svg>
          <div className="cb-donut-center">
            <div style={{ fontSize: 9, letterSpacing: 1.5, color: '#8a7355', textAlign: 'center' }}>{v.donutCenterLabel}</div>
            <div style={{ fontFamily: "'Special Elite',monospace", fontSize: 17, color: '#2c2416', textAlign: 'center', whiteSpace: 'nowrap' }}>{v.donutCenterValue}</div>
          </div>
        </div>
      </div>
      {!v.hasEntries && <div className="cb-empty" style={{ padding: '20px 0' }}>{t.nothingToChart}</div>}
      {v.pieSegments.map((seg) => (
        <div key={seg.id} className="cb-legend-row" onMouseEnter={seg.onEnter} onMouseLeave={seg.onLeave} style={{ background: seg.legendBg, opacity: seg.legendOpacity }}>
    ...
    <div className="cb-settings-anim">
      ...
      <div className="cb-choice-row" style={{ marginBottom: 18 }}>
        {v.languageOptions.map((lo) => (
          <div key={lo.id} className="cb-choice hover-lift" onClick={lo.select} style={{ background: lo.bg, color: lo.fg }}>{lo.label}</div>
        ))}
      </div>
      ...
      <div className="cb-choice-row" style={{ marginBottom: 22 }}>
        {v.currencyOptions.map((co) => (
          <div key={co.id} className="cb-choice hover-lift" onClick={co.select} style={{ background: co.bg, color: co.fg }}>{co.label}</div>
        ))}
      </div>
      ...
      <div className="cb-panel" style={{ marginBottom: 18 }}>
        <div className="cb-panel-row">
          ...
          <div style={{ display: 'flex', gap: 5 }}>
            {v.thousandsToggle.map((opt) => (
              <div key={String(opt.id)} className="cb-toggle hover-lift press-96" onClick={opt.select} style={{ background: opt.bg, color: opt.fg }}>{opt.label}</div>
            ))}
          </div>
        </div>
          ...
          <div className="cb-panel-row">
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              {v.thousandsCharOptions.map((opt) => (
                <div key={opt.id} className="cb-toggle hover-lift press-96" onClick={opt.select} style={{ background: opt.bg, color: opt.fg }}>{opt.label}</div>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ fontSize: 11, color: '#8a7355', textAlign: 'right' }}>{t.preview}: {v.numberFormatPreview}</div>
      </div>
      ...
      <div className="cb-panel" style={{ marginBottom: 22 }}>
        <div className="cb-rate-row">
          ...
          <input type="number" step="0.0001" min="0" value={s.rates.EUR} onChange={(e) => app.setRate('EUR', e)} className="cb-input" />
        </div>
        <div className="cb-rate-row">
          <div className="cb-rate-label">{t.perUsd} RSD</div>
          <input type="number" step="0.01" min="0" value={s.rates.RSD} onChange={(e) => app.setRate('RSD', e)} className="cb-input" />
        </div>
      </div>

      <div className="cb-section-label">{t.data}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 22 }}>
        <div className="cb-btn-outline hover-lift" onClick={app.exportJson}>{t.exportJson}</div>
        <div className="cb-btn-outline hover-lift" onClick={app.exportCsv}>{t.exportCsv}</div>
        <label className="cb-btn-outline cb-btn-dashed hover-lift">
          {t.importJson}
          <input type="file" accept="application/json,.json" onChange={app.onImportJsonFile} style={{ display: 'none' }} />
        </label>
        <label className="cb-btn-outline cb-btn-dashed hover-lift">
          {t.importCsv}
      ...
      <div className="cb-btn-solid hover-lift press-96" onClick={app.goHome} style={{ marginBottom: 12 }}>{t.done}</div>

      <div className="cb-danger-zone">
        <div className="cb-danger-label">{t.dangerZone}</div>
        <div className="cb-btn-danger hover-danger" onClick={app.openDeleteAll}>{t.deleteAll}</div>
      </div>
    </div>
  );
}
...
function AddSheet({ app, s, v, t }) {
  return (
    <>
      <div className="cb-scrim" onClick={app.requestCloseAdd} style={{ animation: s.sheetClosing ? 'cbFadeIn 0.2s ease-in reverse forwards' : 'cbFadeIn 0.15s ease-out' }} />
      <div className="cb-sheet" style={{ animation: s.sheetClosing ? 'cbSheetDown 0.22s ease-in forwards' : 'cbSheetUp 0.22s ease-out' }}>
        <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0 0' }}><div className="cb-sheet-handle" /></div>
        <div style={{ padding: '12px 20px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ fontFamily: "'Special Elite',monospace", fontSize: 19 }}>{s.editingId ? t.editEntry : t.newEntry}</div>
            <div className="cb-round-btn hover-delete" onClick={app.requestCloseAdd}>✕</div>
          </div>

          <div className="cb-field-label">{t.amount}</div>
          <input type="number" step="0.01" min="0" placeholder="0.00" value={s.addAmount} onChange={app.onAmountInput} className="cb-input cb-input-amount" />
          ...
          <div className="cb-category-select hover-lift" onClick={app.toggleCategoryPicker}>
            <div key={v.selectedCatObj.id} className="cb-coin-select-face" style={{ background: coinFace(v.selectedCatObj.color) }}>
          ...
          {s.categoryPickerOpen && (
            <div className="cb-category-grid">
              {v.categoriesForPicker.map((cat) => (
                <div key={cat.id} className="cb-category-chip hover-lift" onClick={cat.select} style={{ background: cat.chipBg, color: cat.chipFg }}>
              ...
              <div className="cb-category-chip cb-category-chip-new hover-lift" onClick={app.startNewCategory}>{t.newChip}</div>
            </div>
          )}

          {s.addingCategory && (
            <div className="cb-new-cat-panel">
              <input type="text" placeholder={t.newCatPlaceholder} value={s.newCatName} onChange={app.onNewCatNameInput} className="cb-input" style={{ marginBottom: 8 }} />
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                {v.swatches.map((sw, i) => (
                  <div key={i} className="cb-swatch hover-swatch" onClick={sw.select} style={{ background: sw.hex, border: sw.border }} />
                ))}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <div className="cb-btn-solid cb-btn-flex hover-lift" onClick={app.confirmNewCategory}>{t.add}</div>
                <div className="cb-btn-outline cb-btn-flex hover-lift" onClick={app.cancelNewCategory}>{t.cancel}</div>
              </div>
            </div>
          )}

          <div className="cb-field-label">{t.memo}</div>
          <input type="text" placeholder={t.memoPlaceholder} value={s.addNote} onChange={app.onNoteInput} className="cb-input cb-input-memo" />

          <div className="cb-stamp-btn hover-stamp" onClick={app.submitAdd}>{s.editingId ? t.saveChanges : t.stampItIn}</div>
        </div>
      </div>
    </>
  );
