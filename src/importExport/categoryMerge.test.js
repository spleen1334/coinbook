...
describe('mergeImportedCategories', () => {
  it('merges a new category by name', () => {
    const { categories, catByName } = mergeImportedCategories(current, [{ name: 'Travel' }], swatches);
  ...
  it('deduplicates by case-insensitive name against existing categories', () => {
    const { categories, catByName } = mergeImportedCategories(current, [{ name: 'food' }], swatches);
    expect(categories).toHaveLength(1);
    expect(catByName.get('food')).toBe('food');
  });

  it('deduplicates imported categories against each other', () => {
    const { categories } = mergeImportedCategories(current, [{ name: 'Travel' }, { name: 'travel' }], swatches);
  ...
  it('preserves a safe source id when it does not collide', () => {
    const { categories } = mergeImportedCategories(current, [{ id: 'travel', name: 'Travel' }], swatches);
  ...
  it('accepts "__proto__" as a plain string id without polluting anything (safe because ids live in a Map)', () => {
    const { categories, catById } = mergeImportedCategories(current, [{ id: '__proto__', name: 'Sketchy' }], swatches);
  ...
  it('does not let a formula-like or overlong source id through', () => {
    const { categories } = mergeImportedCategories(current, [{ id: '=cmd|/c calc', name: 'Formula' }], swatches);
    expect(categories.find((c) => c.name === 'Formula').id).toMatch(/^custom_/);
  });

  it('falls back to a swatch when color is missing or invalid', () => {
    const { categories } = mergeImportedCategories(current, [{ name: 'NoColor' }], swatches);
    expect(swatches).toContain(categories.find((c) => c.name === 'NoColor').color);

    const { categories: categories2 } = mergeImportedCategories(
      current,
      [{ name: 'BadColor', color: 'javascript:alert(1)' }],
      swatches
    );
    expect(swatches).toContain(categories2.find((c) => c.name === 'BadColor').color);
  });

  it('accepts a valid hex color', () => {
    const { categories } = mergeImportedCategories(current, [{ name: 'Hex', color: '#123456' }], swatches);
    expect(categories.find((c) => c.name === 'Hex').color).toBe('#123456');
  });

  it('skips entries with an empty or missing name', () => {
    const { categories } = mergeImportedCategories(current, [{ name: '' }, { name: '   ' }, {}], swatches);
    expect(categories).toHaveLength(1);
  });

  it('does not throw on non-object array entries', () => {
    expect(() => mergeImportedCategories(current, [null, 'garbage', 42, { name: 'Fine' }], swatches)).not.toThrow();
    const { categories } = mergeImportedCategories(current, [null, 'garbage', 42, { name: 'Fine' }], swatches);
    expect(categories.map((c) => c.name)).toContain('Fine');
  });

  it('does not pollute Object.prototype via a __proto__ category name', () => {
    mergeImportedCategories(current, [{ name: '__proto__' }], swatches);
    expect({}.polluted).toBeUndefined();
  });
});
